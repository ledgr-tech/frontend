...
- **Fonts:** Inter for body and UI (`--font-body`, `--font-heading`), Cormorant Garamond as `--font-display` for landing accents and, inside the app shell, every title from ~20px up (`h1`–`h3`, big figures — see the "Tipografia do app" block in `globals.css`), with lining figures. All loaded via `next/font/google` in `app/layout.tsx`. Don't reintroduce the default Geist fonts.
