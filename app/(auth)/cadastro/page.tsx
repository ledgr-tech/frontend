...
import { entrarNaDemonstracao } from "../acoes";
...
// fim do cadastro sem a demonstração: o mascote sentado, esperando a vez
const MASCOTE_FECHADO = { src: "/mascotes/mascote-sentado.png", largura: 1000, altura: 1000 };
...
export default function CadastroPage() {
  ...
  // o backend ainda não cria conta: sem a demonstração, o cadastro termina dizendo isso
  ...
  // mesma razão: o leitor de tela anuncia que o cadastro terminou fechado
  useEffect(() => {
    if (fechado) titulo.current?.focus();
  }, [fechado]);
  ...
  function avancar() {
    ...
    if (ultimoPasso) {
      ...
      timer.current = setTimeout(async () => {
        ...
        // cadastro. Onde a demonstração está ligada, quem entra é a conta de
        // teste, a mesma do login; no resto, o cadastro avisa que está fechado.
        // Quando o endpoint existir, é aqui que os valores coletados viram uma
        // conta de verdade.
        const entrou = demoAberta() && (await entrarNaDemonstracao(true).catch(() => false));
        if (entrou) {
  ...
  const mascote = fechado ? MASCOTE_FECHADO : passo.mascote;
  ...
  return (
    <MolduraAuth rotulo="Cadastro" larguraConteudo={884} linhaDireitaNaBorda>
      ...
      <form className="login-form" onSubmit={enviar} noValidate style={{ width: "100%", maxWidth: 424 }}>
        <div className="login-eyebrow" style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: "clamp(10px, 2vh, 20px)" }}>
          <span className="eyebrow" style={{ fontSize: 12, letterSpacing: "0.14em", textTransform: "uppercase", color: "var(--color-accent-700)" }}>
            {fechado ? "Em testes" : passo.eyebrow}
          </span>
        ...
        <TituloDigitado
          key={fechado ? "fechado" : passo.id}
          ref={titulo}
          texto={fechado ? "Cadastro ainda fechado" : passo.titulo}
          style={{
        ...
        <p className="cadastro-texto" style={{ margin: "0 0 clamp(16px, 2.6vh, 24px)", fontSize: 15, lineHeight: 1.65, color: "color-mix(in srgb, var(--color-text) 72%, transparent)" }}>
          {fechado ? (
            <>
              O Ledgr está em testes com um grupo pequeno de empresas. Para participar, escreva para{" "}
              <a href="mailto:ledgrtech@gmail.com" className="login-link-animado">
                ledgrtech@gmail.com
              </a>
              .
            </>
      ...
      <aside className="cadastro-lado">
        <div key={fechado ? "fechado" : passo.id} className="cadastro-mascote-fundo" aria-hidden="true">
          <Image
